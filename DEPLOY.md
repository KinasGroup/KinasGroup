...
SOURCE database/seed_corporate_accounts.sql;
```
The first script drops the existing `kinas_group` database and creates a fresh one with all 14 tables. The second script seeds the company's two corporate accounts (super admin + super agent) and four sample listings — one per division — so the front page isn't empty on first load.

**Seeded accounts (CHANGE PASSWORDS IMMEDIATELY in production):**
- Super Admin: `admin@kinasgroup.com` / `SuperAdmin@2026`
- Super Agent: `listings@kinasgroup.com` / `SuperAgent@2026` (auto-approved, can list on all 4 divisions without going through the KYC flow)
