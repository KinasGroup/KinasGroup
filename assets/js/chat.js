...
//
// UNREAD VISIBILITY (enforced): incoming messages that are still unread
// get .is-unread on their row so the CSS renders them darker blue in the
// stack; the class is removed automatically once mark-read confirms them.
(function () {
'use strict';
var root = document.getElementById('chatRoot');
 ...
 function applyMessages(messages, initial) {
     ...
     messages.forEach(function (m) {
         var id = m.id;
         if (state.msgEls[id]) {
             // Existing: refresh read-receipt ticks AND the unread
             // highlight (clears the darker blue once marked read).
