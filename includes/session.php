...
class SessionManager {
    ...
    /**
     * Map a listing_type (the value the API/form uses) to the
     * users.division ENUM value stored in the DB.
     */
    public static function listingTypeToDivision(string $listingType): ?string {
        $map = [
            'car'         => 'automobile',
        ...
        return $map[$listingType] ?? null;
    }

    /**
     * True if the current user is allowed to create a listing of the
     * given type. Enforces:
     *   - Admin → any division
     *   - Super agent (agent_profiles.division = 'all') → any division
     *   - Regular agent → only the division they registered for
     *   - Non-agent → never
     */
    public static function canCreateListingInDivision(string $listingType): bool {
        if (!self::isLoggedIn()) return false;
        $role = self::getUserRole();
        $division = self::listingTypeToDivision($listingType);
        if (!$division) return false;

        if ($role === 'admin') return true;
        if ($role !== 'agent') return false;
        if (!self::canCreateListings()) return false;

        try {
            $db = Database::getInstance()->getConnection();
            $stmt = $db->prepare("SELECT division FROM agent_profiles WHERE user_id = ?");
            $stmt->execute([self::getUserId()]);
            $agentDivision = (string)$stmt->fetchColumn();
            // Super-agent override
            if ($agentDivision === 'all') return true;
            return $agentDivision === $division;
        } catch (Throwable $e) {
            return false;
        }
    }
    ...
    /**
     * Returns the divisions the current user is allowed to post in.
     * - Admin: all 4
     * - Super agent (division='all'): all 4
     * - Regular agent: just their one division
     * - Non-agent: empty array
     */
    public static function getAllowedDivisions(): array {
        $all = ['automobile', 'real_estate', 'solar', 'marketplace'];
