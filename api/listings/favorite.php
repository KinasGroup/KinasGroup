...
function showSuccessBanner(message, isError = false) {
    const existingBanners = document.querySelectorAll('.custom-banner');
        ...
        <div style="background:#fff;border-radius:16px;padding:30px;max-width:560px;width:90%;max-height:90vh;overflow-y:auto;position:relative;box-shadow:0 20px 60px rgba(0,0,0,0.3);">
            ...
            <div style="display:flex;align-items:center;gap:15px;padding:15px;background:#f9f9f9;border-radius:8px;margin-bottom:20px;">
                <div style="width:50px;height:50px;border-radius:50%;background:#C6A43F;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:600;font-size:18px;flex-shrink:0;">
                    <span id="schedule-agent-initial"><?= substr($agentName, 0, 1) ?></span>
                </div>
                <div>
                    ...
                    <p style="color:#888;font-size:13px;margin:2px 0 0;">
                        <span>Williams Connect Home</span> · 
                        <span><?= $listingTitle ?></span>
                    </p>
                </div>
            </div>
            
            <form id="schedule-form" onsubmit="submitScheduleForm(event)">
...
function showContactModal(listingId, listingType, agentId, agentName, division) {
    // Remove existing modal if any
    const existing = document.getElementById('contact-agent-modal');
    if (existing) existing.remove();
    ...
    const verifiedBadge = <?= $agentVerified ?> ? '<span style="color:#1B5E20;font-weight:600;">✓ Verified</span>' : '';
        ...
        <div style="max-width:520px;background:#fff;border-radius:16px;padding:30px;width:90%;max-height:90vh;overflow-y:auto;position:relative;box-shadow:0 20px 60px rgba(0,0,0,0.3);">
            ...
            <form id="contact-form" onsubmit="submitContactForm(event)">
                ...
                <div style="margin-bottom:15px;">
                    ...
                    <input type="text" name="subject" id="contact-subject" placeholder="Inquiry about your listing" style="width:100%;padding:10px 14px;border:1px solid #ddd;border-radius:8px;font-size:14px;" value="Inquiry about property">
                </div>
                ...
                <div id="contact-error" style="display:none;margin-top:15px;padding:12px;background:#f8d7da;color:#721c24;border-radius:8px;"></div>
                <div id="contact-success" style="display:none;margin-top:15px;padding:12px;background:#d4edda;color:#155724;border-radius:8px;"></div>
            </form>
        </div>
    `;
