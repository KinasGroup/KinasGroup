...
*
* AMENDED (features JSON fix): the *_listings.features columns are
* MySQL JSON type. A raw comma-separated string ("A, B, C") is NOT
* valid JSON and crashed car inserts with SQLSTATE[22032] 3140
* "Invalid JSON text". All divisions now route features (and property
* amenities) through kinas_normalize_features(), which always stores a
* valid JSON array (or NULL when empty).
*/
...
/**
 * Normalize a "features" value for a MySQL JSON column.
 *
 * Accepts:
 *   - PHP array            -> stored as JSON array
 *   - JSON array string    -> re-encoded as clean JSON array
 *   - comma-separated str  -> split + trimmed + stored as JSON array
 *   - empty / blank        -> NULL
 *
 * Always returns either a valid JSON array string or null, which is
 * exactly what a JSON column accepts. This is the guard that prevents
 * SQLSTATE[22032] 3140 "Invalid JSON text".
 */
function kinas_normalize_features($value): ?string
{
    if ($value === null) {
        return null;
    }
    ...
    // Already a JSON array string? Re-encode it cleanly.
    if ($value[0] === '[') {
        $decoded = json_decode($value, true);
        if (is_array($decoded)) {
            return kinas_normalize_features($decoded);
        }
        return null; // looked like JSON but was invalid — never pass raw
    }
...
$s('interior_color', 50),
$int('seats'),
kinas_normalize_features($data['features'] ?? null),
...
$description,
kinas_normalize_features($data['features'] ?? null),
kinas_normalize_features($data['amenities'] ?? null),
$s('view_type', 100),
$num('hoa_fees'),
$num('inspection_fee'),
$listingStatus, // 'active' for verified, 'pending' for unverified
