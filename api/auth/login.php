...
try {
    ...
    // AMENDED: Added deleted_by to the SELECT so we can distinguish
    // self-deleted accounts (reactivatable) from admin-deleted accounts.
    $stmt = $db->prepare(
    ...
    // ============================================================
    // Self-deleted accounts are allowed to proceed to the
    // reactivation page instead of being blocked outright.
    // Admin-deleted accounts remain blocked.
    // ============================================================
    if ($status === 'deleted') {
        ...
        if ($deletedBy === 'self') {
            ...
            // Set pending reactivation flags
        ...
        login_json_error(403, 'Your account has been deleted by an administrator. Please contact support.');
    }
