...
// AMENDED: the identifier may be an email address OR a username.
$identifier = strtolower(trim($data['email'] ?? $data['username'] ?? ''));
...
http_response_code(400);
echo json_encode(['error' => 'Please enter both email/username and password.']);
exit;
}
...
// AMENDED: lookup by email OR by username (public identity).
$stmt = $db->prepare(
"SELECT id, name, username, email, password, role, verified, status, email_verified_at FROM users WHERE " . ($isEmail ? "email = ?" : "username = ?")
);
...
Security::logActivity($user['id'] ?? null, 'login_failed', "Failed login attempt for: $identifier from $ip");
http_response_code(401);
echo json_encode(['error' => 'Invalid email/username or password. Please try again.']);
exit;
}
// Check user status
...
Security::logActivity($user['id'], 'login_blocked_unverified', "Login blocked — email not verified. identifier=$identifier from $ip");
http_response_code(403);
echo json_encode([
