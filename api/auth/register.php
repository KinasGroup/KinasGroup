...
// Normalize the public username (lowercase, strip leading @) before validation
$data['username'] = kinas_normalize_username($data['username'] ?? '');
// Validate input
...
// Username format + reserved-name validation (public identity)
$usernameError = kinas_username_error($data['username']);
...
if ($data['username'] !== '' && kinas_username_taken($db, $data['username'])) {
http_response_code(409);
...
$stmt = $db->prepare("
INSERT INTO users
(name, username, email, password, phone, registration_ip, duplicate_flag_reason, role, status,
