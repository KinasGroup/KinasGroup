...
* - The provider is presented publicly as @username (provider card,
*   contact modal, schedule modal). The legal name stays private
*   (DB/admin/KYC).
*/
...
$stmt = $db->prepare("
SELECT s.*, a.verified as agent_verified, a.name as agent_name, a.username as agent_username, a.email as agent_email, a.phone as agent_phone,
...
$agentPublicName = htmlspecialchars(kinas_public_display_name($item['agent_username'] ?? null, $item['agent_name'] ?? 'Provider'), ENT_QUOTES, 'UTF-8');
...
<!-- Contact Provider (AMENDED: passes the PUBLIC @username identity) -->
<button class="je-cta-secondary" id="contactBtn" onclick="openContactAgent(<?= $agentId ?>, '<?= $agentPublicName ?>', 'solar');">
