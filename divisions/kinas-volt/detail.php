    ...
    <div class="je-detail-grid">
        ...
        <aside class="je-spec-panel">
            ...
            <div class="je-cta-row">
                ...
                <button class="je-cta-primary" onclick="openScheduleViewing(<?= (int)$item['id'] ?>, 'solar', <?= (int)$item['agent_id'] ?>)">
                    <i class="far fa-calendar-alt"></i> Schedule Viewing
                </button>
                
                <!-- Contact Agent - opens contact form (not phone) -->
                <button class="je-cta-secondary" onclick="openContactAgent(<?= (int)$item['agent_id'] ?>, '<?= htmlspecialchars($item['agent_name'] ?? 'Provider') ?>', 'solar')">
