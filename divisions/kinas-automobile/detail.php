...
* - Sold/rented/completed listings remain publicly accessible so verified
...
* - The agent is presented publicly as @username (agent card, contact
*   modal, schedule modal). The legal name stays private (DB/admin/KYC).
*/
...
$stmt = $db->prepare("
SELECT c.*, a.verified as agent_verified, a.name as agent_name, a.username as agent_username, a.email as agent_email, a.phone as agent_phone,
...
//
// Sold / rented / completed / under-offer listings are also kept public
// so that verified customers can still view the product and leave a
// review after the transaction is completed.
//
...
$agentPublicName = htmlspecialchars(kinas_public_display_name($item['agent_username'] ?? null, $item['agent_name'] ?? 'Agent'), ENT_QUOTES, 'UTF-8');
...
<!-- Contact Agent (AMENDED: passes the PUBLIC @username identity) -->
<button class="je-cta-secondary" id="contactBtn" onclick="openContactAgent(<?= $agentId ?>, '<?= $agentPublicName ?>', 'car');">
<i class="far fa-envelope"></i> Contact Agent
</button>
<!-- Save Listing -->
<button class="je-cta-secondary" id="saveBtn" onclick="jeSaveListing('car', <?= $listingId ?>);">
...
kinas_render_reviews_section($db, 'car', $listingId);
}
}
?>
<?php endif; ?>
...
<strong><?= $agentPublicName ?></strong> · <?= $listingTitle ?>
</div>
...
console.log('=== KINAS AUTOMOBILE detail page loaded ===');
</script>
