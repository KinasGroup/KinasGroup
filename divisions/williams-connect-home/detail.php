<?php
/**
 * KINAS BUILD: 2026.08.15.07
 * FILE: divisions/williams-connect-home/detail.php
 *
 * WILLIAMS CONNECT HOME — Property detail
 *
 * RESTORED:
 * - Inline Contact Agent modal.
 * - Paid scheduling / inspection fee flow.
 * - Virtual tour section for uploaded videos, YouTube/Vimeo and external links.
 * - Mortgage calculator.
 ...
 *
 * VISIBILITY:
 * - active, sold and rented listings are visible publicly.
 * - other statuses are visible only to owner/admin as preview.
 *
 * REVIEWS:
 * - Review engine is loaded only if it exists.
 * - Review system itself is left untouched.
 */
...
// - rented
//
// Other statuses are only visible to owner/admin as preview.
// ============================================================
...
if (!empty($item['amenities'])) {
    $features = array_merge(
        $features,
        is_array($item['amenities'])
            ? $item['amenities']
            : (json_decode((string)$item['amenities'], true) ?: [])
    );
}
...
$listingTitleRaw = $item['title'] ?? 'Property';
    ...
    <div class="je-detail-wrap">
        ...
        <?php if ($isPreview): ?>
            <div style="background:#FFF8E1; border:1px solid #F0C419; color:#7A5B00; padding:14px 18px; border-radius:4px; margin-bottom:20px; font-size:14px;">
                ...
                <strong>Preview only</strong> — this listing is <?= htmlspecialchars(ucfirst((string)($item['status'] ?? ''))) ?> and not visible to the public.
            </div>
        <?php endif; ?>

        <?php if (!$isPreview && in_array((string)($item['status'] ?? ''), ['sold', 'rented'], true)): ?>
                ...
                <?php else: ?>
                    <div class="je-gallery-main">
                        <img id="jeMainImage"
                             ...
                             alt="<?= htmlspecialchars($item['title'] ?? 'Property') ?>"
                             onerror="this.onerror=null; this.src='/assets/images/placeholder/property-placeholder.svg';">
                    </div>
            ...
            <aside class="je-spec-panel">
                ...
                <div class="je-spec-price">
                    ...
                    if (function_exists('formatPrice')) {
                        echo formatPrice((float)($item['price'] ?? 0));
                    } else {
                        echo '₦' . number_format((float)($item['price'] ?? 0));
                    }
                    ?>

                    <?php if (($item['listing_type'] ?? '') === 'rent'): ?>
                ...
                <dl class="je-spec-key">
                    ...
                    $keys = [
                        ...
                        'HOA Fees'    => ($item['hoa_fees'] ?? null) !== null ? (function_exists('formatPrice') ? formatPrice((float)$item['hoa_fees']) : '₦' . number_format((float)$item['hoa_fees'])) . '/mo' : null,
                ...
                <div class="je-cta-row">
                    <?php if ((string)($item['status'] ?? '') === 'active'): ?>
                        <button class="je-cta-primary"
                                id="scheduleBtn"
                                onclick="window.openScheduleViewing(<?= $listingId ?>, 'property', <?= $agentId ?>, <?= $inspectionFee ?>);">
                            <i class="far fa-calendar-alt"></i> Schedule Viewing
                        </button>

                        <button class="je-cta-secondary"
                                id="contactBtn"
                                onclick="window.openContactAgent(<?= $agentId ?>, <?= htmlspecialchars(json_encode($agentNameRaw, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP), ENT_QUOTES, 'UTF-8') ?>, 'Real Estate');">
                    ...
                    <button class="je-cta-secondary"
                            id="saveBtn"
                            onclick="window.jeSaveListing('property', <?= $listingId ?>);">
        ...
        <section class="je-section" style="padding-left:0;padding-right:0;border-top:1px solid #e8e8e8; margin-top:40px;">
            ...
            <?php
            $tourUrl = (string)($item['virtual_tour_url'] ?? '');
            $tourType = (string)($item['virtual_tour_type'] ?? '');
            $tourThumbnail = (string)($item['virtual_tour_thumbnail'] ?? '');
            ...
            <?php if ($tourType === 'video' && $tourUrl): ?>
                <video controls
                       preload="metadata"
                       <?= $tourThumbnail ? 'poster="' . htmlspecialchars($tourThumbnail) . '"' : '' ?>
                       style="width:100%;max-height:480px;border-radius:8px;background:#000;">
                    <source src="<?= htmlspecialchars($tourUrl) ?>">
                    Your browser doesn't support embedded video.
                    <a href="<?= htmlspecialchars($tourUrl) ?>" target="_blank" rel="noopener">Watch it directly</a> instead.
                </video>
            <?php elseif ($embedUrl): ?>
            ...
            <?php elseif ($tourType === 'link' && $tourUrl): ?>
                <a href="<?= htmlspecialchars($tourUrl) ?>"
                   target="_blank"
                   rel="noopener"
                   style="display:flex;align-items:center;justify-content:center;gap:10px;background:#151515;color:#fff;border-radius:8px;padding:20px;text-decoration:none;font-weight:600;">
            ...
            <div class="je-finance-calc" style="background:#f9f8f5;border:1px solid #e8e8e8;border-radius:8px;padding:24px;max-width:520px;">
                ...
                <p style="margin:10px 0 0;font-size:11px;color:#999;">
                    Estimate only. Actual mortgage terms are set by your chosen lender and subject to approval.
                </p>
            </div>
        ...
        <!-- ============================================================ -->
        <!-- REVIEWS — ENGINE LEFT UNTOUCHED -->
        <!-- ============================================================ -->
        <?php if (!$isPreview): ?>
            <?php
            $review_listing_type = 'property';
            $review_listing_id = $listingId;
