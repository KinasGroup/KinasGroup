...
// ============================================================
// RECENT MESSAGES
// AMENDED: join username so conversations display @username.
// ============================================================
$stmt = $db->prepare("
SELECT m.*, u.name AS other_name, u.username AS other_username
FROM messages m
...
$stmt->execute([$user_id]);
$user = $stmt->fetch();
// AMENDED: public identity for the welcome banner.
$userDisplayName = kinas_public_display_name($user['username'] ?? null, $user['name'] ?? '');
...
<div>
<h1>Welcome back, <?php echo htmlspecialchars($userDisplayName); ?>! 👋</h1>
...
// AMENDED: prefer @username for the other party's display name.
$otherName = kinas_public_display_name(
$msg['other_username'] ?? null,
$msg['other_name'] ?? ($isSender ? 'Agent' : 'User')
);
