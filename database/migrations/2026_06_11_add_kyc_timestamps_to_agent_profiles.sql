...
-- check, so MySQL skips columns that are already present.
-- (MySQL 8 does NOT support `ADD COLUMN IF NOT EXISTS` — only
--  MariaDB does — so we use the portable pattern below.)
-- =====================================================================
...
SET @stmt := IF(@col = 0,
    'ALTER TABLE agent_profiles ADD COLUMN kyc_submitted_at TIMESTAMP NULL COMMENT ''When the agent submitted their KYC application'' AFTER kyc_verification_id',
    'SELECT 1'
);
...
SET @col := (
    SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
     WHERE TABLE_SCHEMA = DATABASE()
       AND TABLE_NAME   = 'agent_profiles'
       AND COLUMN_NAME  = 'kyc_decision_at'
);
SET @stmt := IF(@col = 0,
    'ALTER TABLE agent_profiles ADD COLUMN kyc_decision_at TIMESTAMP NULL COMMENT ''When the final approve/reject decision was made'' AFTER kyc_submitted_at',
    'SELECT 1'
);
PREPARE s FROM @stmt; EXECUTE s; DEALLOCATE PREPARE s;
