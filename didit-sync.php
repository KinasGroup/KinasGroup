 ...
 * the local state would stay stale forever and the verification page
 * would show "unverified" for an agent who IS verified on Didit.
 *
 * These two functions fetch the authoritative decision from Didit and
 * apply the exact same state transitions the webhook applies:
 *   didit_sync_kyc() — identity (KYC)
 *   didit_sync_kyb() — business (KYB)
 *
 * BUSINESS RULES ENFORCED HERE (as per product spec):
 ...
 *   • Name-mismatch on approved KYC => rejected, with reason stored.
 *
 * Safe to call on every page load: no-ops when state is already final,
 * Didit is disabled, or no session exists.
 */
...
function didit_sync_kyc(PDO $db, int $userId): void
{
    ...
    $documentName = null; $nameMatch = null; $mismatch = 0; $reason = null;
    $registered = '';
    if ($mapped === 'approved') {
    ...
    $sets = ['status = ?', 'didit_status = ?', 'completed_at = COALESCE(completed_at, ?)', 'updated_at = NOW()'];
    $params = [$mapped, (string)$fetched['status'], in_array($mapped, ['approved', 'rejected'], true) ? $now : null];
    ...
    if (didit_sync_column_exists($db, 'agent_profiles', 'kyc_decision_at')) {
        ...
        $params[] = in_array($mapped, ['approved', 'rejected'], true) ? 1 : 0;
        $params[] = $now;
    }
    ...
    // ORANGE BADGE RULE: KYC pass => verified=1 for individuals AND
    // businesses (can list except car rentals). Rejected => unverified.
    if ($mapped === 'approved') {
