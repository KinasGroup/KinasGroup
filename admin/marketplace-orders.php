 ...
 *
 * AMENDED:
 * - Connects orders to the product review moderation system.
 * - Shows whether each purchased marketplace item has been reviewed.
 * - Adds shortcut to pending marketplace reviews.
 */
...
if (!empty($orders)) {
    ...
    $itemsStmt = $db->prepare("
        SELECT oi.order_id, oi.listing_id, oi.title, oi.price, oi.agent_id, a.name AS agent_name
        FROM order_items oi
...
try {
    ...
    $pendingMarketplaceReviews = (int)$stmt->fetchColumn();
} catch (Throwable $e) {
    $pendingMarketplaceReviews = 0;
}
...
if (!empty($orders)) {
    ...
    if (!empty($listingIds) && !empty($buyerIds)) {
        try {
            $listingIn = implode(',', array_fill(0, count($listingIds), '?'));
            $buyerIn = implode(',', array_fill(0, count($buyerIds), '?'));
            ...
            $stmt = $db->prepare("
                ...
                WHERE listing_type = 'marketplace'
                  ...
                  AND user_id IN ($buyerIn)
            ");

            $stmt->execute(array_merge($listingIds, $buyerIds));

            foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $reviewRow) {
                $key = ((int)$reviewRow['user_id']) . '-' . ((int)$reviewRow['listing_id']);
                $reviewMap[$key] = (string)$reviewRow['status'];
            }
        } catch (Throwable $e) {
...
function marketplace_order_review_badge(?string $status, int $listingId): string
{
    $href = 'reviews.php?tab=reviews&status=all&q=' . $listingId;

    if ($status === null) {
        return '<a class="review-pill none" href="' . htmlspecialchars($href) . '" title="Open review moderation">'
            . '<i class="far fa-comment-dots"></i> No review'
            . '</a>';
    }

    if ($status === 'approved') {
        return '<a class="review-pill approved" href="' . htmlspecialchars($href) . '" title="Open review moderation">'
            . '<i class="fas fa-check-circle"></i> Approved'
            . '</a>';
    }

    if ($status === 'pending') {
        return '<a class="review-pill pending" href="' . htmlspecialchars($href) . '" title="Open review moderation">'
            . '<i class="fas fa-clock"></i> Pending'
            . '</a>';
    }

    if ($status === 'rejected') {
        return '<a class="review-pill rejected" href="' . htmlspecialchars($href) . '" title="Open review moderation">'
            . '<i class="fas fa-times-circle"></i> Rejected'
            . '</a>';
    }

    return '<a class="review-pill none" href="' . htmlspecialchars($href) . '" title="Open review moderation">'
        . '<i class="far fa-comment-dots"></i> Unknown'
        . '</a>';
}
...
<style>
    ...
    .settle-pill.subaccount {
        color: #2E7D32;
    }

    .settle-pill.platform {
        color: #999;
    }
...
<div class="je-dash-shell" style="max-width:100%;overflow-x:hidden;">
    ...
    <main class="je-dash-main" style="overflow-x:hidden;width:100%;max-width:100%;padding:15px;">
        <div class="page-header">
            <div>
                <h1>
                    <i class="fas fa-receipt" style="color:#C6A43F;margin-right:10px;"></i>
                    Marketplace Orders
                </h1>
                <p>
                    Every Paystack checkout through KINAS Marketplace — fees, settlement, and customer review status.
                </p>
            </div>

            <div class="header-actions">
                <a href="reviews.php?tab=reviews&status=pending&type=marketplace" class="btn-reviews">
                    <i class="fas fa-star"></i>
                    Product Reviews
                    <?php if ($pendingMarketplaceReviews > 0): ?>
                        (<?= (int)$pendingMarketplaceReviews ?> pending)
                    <?php endif; ?>
                </a>
            </div>
        </div>
        ...
        <div class="stats-grid">
            <div class="stat-card">
                ...
                <p>Total checkout attempts</p>
            </div>

            <div class="stat-card">
                <h3><?= number_format((int)$stats['paid_orders']) ?></h3>
                <p>Paid orders</p>
            </div>

            <div class="stat-card">
                <h3>₦<?= number_format((float)$stats['gross_revenue']) ?></h3>
                <p>Gross revenue collected</p>
            </div>

            <div class="stat-card">
                <h3>₦<?= number_format((float)$stats['fees_passed_to_buyers']) ?></h3>
                <p>Paystack fees passed to buyers</p>
            </div>

            <div class="stat-card">
                <h3><?= number_format((int)$stats['auto_settled_orders']) ?></h3>
                <p>Auto-settled to agents</p>
            </div>

            <div class="stat-card">
                <h3><?= number_format((int)$stats['manual_settle_orders']) ?></h3>
                <p>Pending manual payout</p>
            </div>

            <div class="stat-card">
                <h3><?= (int)$pendingMarketplaceReviews ?></h3>
                <p>Pending marketplace reviews</p>
            </div>
        </div>
                            ...
                            <?php foreach ($orders as $o): $items = $itemsByOrder[$o['id']] ?? []; ?>
                                <tr>
                                    ...
                                    <td>
                                        <?php foreach ($items as $it): ?>
                                            <div class="item-line">
                                                ...
                                                <span class="seller">
                                                    — <?= htmlspecialchars($it['agent_name'] ?? 'agent #' . $it['agent_id']) ?>
                                                </span>
                                            </div>
                                        <?php endforeach; ?>
                                    </td>
                                    ...
                                    <td>
                                        <?php foreach ($items as $it): ?>
                                            <?php
                                            $reviewKey = ((int)$o['buyer_id']) . '-' . ((int)($it['listing_id'] ?? 0));
                                            $reviewStatus = $reviewMap[$reviewKey] ?? null;
                                            ?>
                                            <div class="review-line">
                                                <?= marketplace_order_review_badge($reviewStatus, (int)($it['listing_id'] ?? 0)) ?>
                                            </div>
                                        <?php endforeach; ?>
                                    </td>
                                    ...
                                    <td>
                                        ...
                                        <?php if ((float)($o['fee_amount'] ?? 0) > 0): ?>
                                            <br>
                                            <span style="font-size:11px;color:#999;">
                                                (subtotal ₦<?= number_format((float)$o['subtotal_amount']) ?>)
                                            </span>
                                        <?php endif; ?>
                                    </td>

                                    <td>
                                        ₦<?= number_format((float)($o['fee_amount'] ?? 0)) ?>
                                    </td>

                                    <td>
                                        <?php if (($o['settlement_mode'] ?? 'platform') === 'subaccount'): ?>
                                            <span class="settle-pill subaccount">
                                                <i class="fas fa-bolt"></i> Auto (<?= htmlspecialchars($o['subaccount_code']) ?>)
                                            </span>
                                        <?php else: ?>
