...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($adminAction === 'moderate_review') {
            ...
            $stmt = $db->prepare("
                SELECT id, listing_type, listing_id, status
            ...
            if ($moderationAction === 'delete') {
                if ($reportsInstalled) {
                    $db->prepare("DELETE FROM product_review_reports WHERE review_id = ?")
                        ->execute([$reviewId]);
                }

                $db->prepare("DELETE FROM product_reviews WHERE id = ?")
                    ->execute([$reviewId]);
            ...
            admin_reviews_success_redirect(
                'reviews',
                $newStatus === 'approved'
                    ? 'Review approved and is now visible on the product page.'
                    : 'Review rejected and hidden from the product page.'
            );
        }
            ...
            if ($reportAction === 'reject_review' && $linkedReviewId > 0) {
                ...
                if (in_array('moderation_note', $reviewColumns, true)) {
                    ...
                    $params[] = $moderationNote !== ''
                        ? $moderationNote
                        : 'Rejected after customer report.';
                }
        ...
        if ($adminAction === 'add_verified_purchase') {
            ...
            Security::logActivity(
                ...
                sprintf(
                    'Manually verified user #%d for %s listing #%d',
                    $verifiedUserId,
                    $listingType,
                    $listingId
                )
...
if (!in_array($typeFilter, ['all', 'car', 'property', 'solar', 'marketplace'], true)) {
    $typeFilter = 'all';
}
...
if ($reviewsInstalled) {
    ...
    if ($searchQuery !== '') {
        $where[] = '(r.comment LIKE ? OR u.name LIKE ? OR u.email LIKE ? OR r.listing_id = ?)';
...
if (!in_array($reportStatusFilter, ['open', 'resolved', 'dismissed', 'all'], true)) {
...
<style>
    ...
    .kr-verified-form input,
    .kr-verified-form select {
        width: 100%;
        padding: 10px 12px;
        border: 1px solid #DDD;
        border-radius: 8px;
...
<div class="je-dash-shell kr-admin-wrap">
    ...
    <main class="je-dash-main" style="overflow-x:hidden;width:100%;max-width:100%;padding:15px;">
        <div class="kr-admin-header">
            <h1>
                <i class="fas fa-star" style="color:#C6A43F;margin-right:10px;"></i>
                Product Reviews
            </h1>
            <p>
                Moderate customer product reviews, manage reported reviews, and control verified purchase access.
            </p>
        </div>

        <?php if (!empty($flashSuccess)): ?>
            <div class="kr-flash success">
                <i class="fas fa-check-circle"></i> <?= htmlspecialchars($flashSuccess) ?>
            </div>
        <?php endif; ?>

        <?php if (!empty($flashError)): ?>
            <div class="kr-flash error">
        ...
        <?php if (!$reviewsInstalled): ?>
            <div class="kr-flash error">
                The <strong>product_reviews</strong> table is not installed. Run the review system database migration first.
            </div>
        <?php endif; ?>

        <div class="kr-tabs">
            <a href="?tab=reviews&status=<?= htmlspecialchars($statusFilter) ?>&type=<?= htmlspecialchars($typeFilter) ?>&q=<?= urlencode($searchQuery) ?>"
               class="<?= $tab === 'reviews' ? 'active' : '' ?>">
                <i class="fas fa-comments"></i>
                Reviews
                <span class="kr-count-pill"><?= (int)($stats['pending'] + $stats['approved'] + $stats['rejected']) ?></span>
            </a>
            ...
            <a href="?tab=reports"
               class="<?= $tab === 'reports' ? 'active' : '' ?>">
                ...
                Reports
                <span class="kr-count-pill"><?= (int)$stats['open_reports'] ?></span>
            </a>

            <a href="?tab=verified"
               class="<?= $tab === 'verified' ? 'active' : '' ?>">
                ...
                <div class="kr-card-pad">
                    <form method="GET" class="kr-filters">
                        ...
                        <input type="text"
                               ...
                               placeholder="Search comment, customer, email, listing ID">

                        <button type="submit" class="kr-btn gold">
                            <i class="fas fa-filter"></i> Filter
                        </button>

                        <a href="?tab=reviews" class="kr-btn secondary">
                            Reset
                        </a>
                    </form>
                </div>
                            ...
                            <tbody>
                                <?php foreach ($reviews as $review): ?>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a href="<?= htmlspecialchars($listingUrl) ?>" target="_blank" rel="noopener">
                                                <?= htmlspecialchars($listingTitle) ?>
                                            </a>
                                            <div class="kr-muted">
                                                <?= htmlspecialchars(ucfirst($listingType)) ?> #<?= $listingId ?>
                                            </div>
                                        </td>
                                        ...
                                        <td>
                                            <div class="kr-comment">
                                                <?= nl2br(htmlspecialchars($commentExcerpt)) ?>
                                            </div>

                                            <?php if (!empty($review['moderation_note'])): ?>
                                                <div class="kr-note">
                                                    <i class="fas fa-comment-dots"></i>
                                                    <?= htmlspecialchars((string)$review['moderation_note']) ?>
                                                </div>
                                            <?php endif; ?>
                                        </td>
                                        ...
                                        <td>
                                            <?= date('M j, Y H:i', strtotime((string)($review['created_at'] ?? 'now'))) ?>
                                        </td>

                                        <td>
                                            <?php if (($review['status'] ?? '') !== 'approved'): ?>
                                                <form method="POST" class="kr-inline-form" data-confirm="Approve this review?">
                                                    ...
                                                    <button type="submit" class="kr-btn-sm success">
                    ...
                    <?php if ($totalPages > 1): ?>
                        <div class="kr-pagination">
                            ...
                            <?php if ($page > 1): ?>
                                ...
                                ?>
                                <a class="kr-btn secondary" href="reviews.php?<?= htmlspecialchars(http_build_query($paginationParams)) ?>">
            ...
            <div class="kr-card">
                <div class="kr-card-pad">
                    <form method="GET" class="kr-filters">
                        ...
                        <a href="?tab=reports" class="kr-btn secondary">
                            Reset
                        </a>
                    </form>
                </div>
                ...
                <?php if (!$reportsInstalled): ?>
                    <div class="kr-empty">
                        Review reports are not installed. Run the review system database migration first.
                    </div>
                <?php elseif (empty($reports)): ?>
                    <div class="kr-empty">
                        <i class="fas fa-flag" style="font-size:36px;display:block;margin-bottom:10px;color:#DDD;"></i>
                        No reports found.
                    </div>
                <?php else: ?>
                    <div class="kr-table-wrap">
                        <table class="kr-table">
                            ...
                            <tbody>
                                <?php foreach ($reports as $report): ?>
                                    ...
                                    $reportListingUrl = $reportListingType && $reportListingId
                                        ? admin_reviews_listing_url($reportListingType, $reportListingId)
                                        : '';
                                    ?>
                                    <tr>
                                        <td>
                                            #<?= $reportId ?>
                                            <div class="kr-muted">
                                                <?= date('M j, Y H:i', strtotime((string)($report['created_at'] ?? 'now'))) ?>
                                            </div>
                                        </td>

                                        <td>
                                            <?php if ($linkedReviewId > 0): ?>
                                                <a href="?tab=reviews&status=all&q=<?= $linkedReviewId ?>">
                                                    Review #<?= $linkedReviewId ?>
                                                </a>

                                                <div class="kr-muted">
                                                    Current status:
                                                    <?= htmlspecialchars(ucfirst((string)($report['review_status'] ?? 'unknown'))) ?>
                                                </div>
                                            <?php else: ?>
                                        ...
                                        <td>
                                            <?php if ($reportListingUrl): ?>
                                                <a href="<?= htmlspecialchars($reportListingUrl) ?>" target="_blank" rel="noopener">
                                                    <?= htmlspecialchars(admin_reviews_listing_title($db, $reportListingType, $reportListingId)) ?>
                                                </a>
                                                <div class="kr-muted">
                                                    <?= htmlspecialchars(ucfirst($reportListingType)) ?> #<?= $reportListingId ?>
                                                </div>
                                            <?php else: ?>
                                        ...
                                        <td>
                                            <div class="kr-comment">
                                                <?= nl2br(htmlspecialchars((string)($report['reason'] ?? ''))) ?>
                                            </div>
                                        </td>
                                        ...
                                        <td>
                                            <?php if (($report['status'] ?? '') === 'open'): ?>
                                                <?php if ($linkedReviewId > 0 && ($report['review_status'] ?? '') !== 'rejected'): ?>
                                                    <form method="POST"
                                                          class="kr-inline-form"
                                                          data-confirm="Reject the linked review?"
                                                          data-prompt="Optional moderation note:">
                                                        <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($csrfToken) ?>">
            ...
            <div class="kr-card">
                <div class="kr-card-pad">
                    <h3 style="margin:0 0 12px;">
                        <i class="fas fa-plus-circle" style="color:#C6A43F;"></i>
                        Add Manual Verified Purchase
                    </h3>

                    <p class="kr-muted" style="margin-top:0;">
                        Use this for offline sales, direct bank transfers, completed inspections, or completed rentals where the platform did not automatically record a purchase.
                    </p>

                    <?php if (!$verifiedInstalled): ?>
                        <div class="kr-flash error">
                            Verified purchases are not installed. Run the review system database migration first.
                        </div>
                    <?php else: ?>
                        <form method="POST" class="kr-verified-form">
                            ...
                            <div>
                                ...
                                <input type="email" name="user_email" placeholder="customer@email.com">
                            </div>

                            <div>
                                <label>Or Customer User ID</label>
                                <input type="number" name="user_id" min="1" placeholder="123">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" name="listing_id" min="1" required placeholder="45">
                            </div>

                            <div>
                                <label>Reference / Notes</label>
                                <input type="text" name="order_reference" placeholder="Offline sale, inspection, rental, etc.">
                            </div>

                            <div>
                                <button type="submit" class="kr-btn gold">
                                    <i class="fas fa-badge-check"></i> Add Verified Purchase
                                </button>
                            </div>
                        </form>
                    <?php endif; ?>
                ...
                <?php if ($verifiedInstalled): ?>
                    <?php if (empty($verifiedPurchases)): ?>
                        <div class="kr-empty">
                            <i class="fas fa-badge-check" style="font-size:36px;display:block;margin-bottom:10px;color:#DDD;"></i>
                            No manual verified purchases recorded yet.
                        </div>
                    <?php else: ?>
                        <div class="kr-table-wrap">
                            <table class="kr-table">
                                ...
                                <tbody>
                                    <?php foreach ($verifiedPurchases as $purchase): ?>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <strong><?= htmlspecialchars($purchase['user_name'] ?? 'Unknown') ?></strong>
                                                <div class="kr-muted">
                                            ...
                                            <td>
                                                <a href="<?= htmlspecialchars($purchaseUrl) ?>" target="_blank" rel="noopener">
                                                    <?= htmlspecialchars(admin_reviews_listing_title($db, $purchaseType, $purchaseListingId)) ?>
                                                </a>
                                                <div class="kr-muted">
                                                    <?= htmlspecialchars(ucfirst($purchaseType)) ?> #<?= $purchaseListingId ?>
                                                </div>
                                            </td>

                                            <td>
                                                <?= htmlspecialchars(ucfirst((string)($purchase['source'] ?? 'manual'))) ?>
                                            </td>

                                            <td>
                                                <?= htmlspecialchars((string)($purchase['order_reference'] ?? '—')) ?>
                                            </td>

                                            <td>
                                                <?= htmlspecialchars($purchase['creator_name'] ?? 'System') ?>
                                            </td>
