<?php
/**
 * ADMIN — Solar Calculator Control (DUAL-OPTION rebuild)
 *
 * Tabs:
 * - settings:  admin-controlled calculation assumptions (no fallback pricing)
 * - products:  which KINAS VOLT listings the calculator may quote + specs
 *              (incl. Max PV Input per generator for Strict Mode)
...
// NOTE: default_panel_wattage / default_panel_price were REMOVED.
// The dual-option engine quotes real products only — no reference
// panel, no fallback price, ever.
// ============================================================
$settingsCatalog = [
    'technical' => [
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // ----------------------------------------------------
        // Save product specs (now includes max_pv_input_w)
        // ----------------------------------------------------
        if ($action === 'save_product') {
            ...
            $db->prepare("
                ...
                SET product_type = ?, panel_wattage_w = ?, inverter_capacity_kva = ?, continuous_kw = ?,
                    ...
                    max_pv_input_w = ?, expandable = ?, priority = ?, active = ?
                WHERE id = ?
            ")->execute([
                ...
                (int)max(0, (float)($_POST['max_pv_input_w'] ?? 0)),
        ...
        // ----------------------------------------------------
        // Add product (now includes max_pv_input_w)
        // ----------------------------------------------------
        if ($action === 'add_product') {
            ...
            $db->prepare("
                ...
                (listing_id, product_type, panel_wattage_w, inverter_capacity_kva, continuous_kw,
                 battery_capacity_kwh, usable_battery_kwh, battery_voltage_v, max_pv_input_w, expandable, priority, active)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)
            ")->execute([
                $listingId,
                in_array($_POST['product_type'] ?? '', ['generator', 'panel', 'inverter', 'battery'], true) ? $_POST['product_type'] : 'generator',
...
// Settings (defaults overlaid with DB values) — no fallback pricing keys
$defaults = [
    ...
    'co2_kg_per_kwh' => 0.85, 'electricity_tariff_ngn' => 225.0,
];
try {
...
table.sc-table{width:100%;min-width:1100px;border-collapse:collapse}
...
<p>Control the dual-option quotation engine: settings, approved products (with Max PV Input), and proposals.</p>
</div>
...
<div class="sc-note">Quotations use LIVE product prices only. Reference/fallback panel pricing has been removed — the calculator never invents a price. Service costs (installation, cabling, mounting, transport) are intentionally NOT part of the automatic quotation — the company does not currently offer them.</div>
</div>
<div class="sc-card-pad" style="border-top:1px solid #F0F0F0;">
...
<div class="sc-field"><label>Max PV Input (W) — generators</label><input type="number" name="max_pv_input_w" value="0" min="0"></div>
...
<div class="sc-note">Prices come LIVE from the listing. Confirm the continuous kW / usable kWh values against the real datasheets. <strong>Strict Mode:</strong> a generator with Max PV Input = 0 is excluded from Option 1 (All-in-One) quotes until its real panel-input cap is entered.</div></div>
...
<thead><tr><th>Product</th><th>Type</th><th>Panel W</th><th>kVA</th><th>kW</th><th>Batt kWh</th><th>Usable kWh</th><th>V</th><th>Max PV W</th><th>Priority</th><th>Status</th><th>Actions</th></tr></thead>
<tbody>
...
<td><input type="number" name="max_pv_input_w" value="<?= (int)($p['max_pv_input_w'] ?? 0) ?>" style="width:70px;"></td>
