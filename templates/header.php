 ...
 *
 * AMENDED: Notification system upgraded:
 * - Facebook-style louder notification sound
 * - 5-second polling for near-instant detection
 * - Mobile hamburger menu badge (visible without opening menu)
 * - Floating toast notification on new message
 */
...
if ($isLoggedIn && $userUsername === '') {
    try {
        ...
        $userUsername = (string)($__st->fetchColumn() ?: '');
...
if ($scriptName === 'index.php' && preg_match('#^/divisions/[^/]+/?$#', $requestUri)) {
    $isHeroPage = true;
}
if ($scriptName === 'about.php' && preg_match('#^/pages/about\.php$#', $requestUri)) {
    $isHeroPage = true;
}
...
<?php if ($whatsappEnabled && $whatsappLoggedIn): ?>
<script>
window.SITE_CONSTANTS = window.SITE_CONSTANTS || {};
window.SITE_CONSTANTS.WHATSAPP_NUMBER = <?= json_encode($whatsappNumber, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP) ?>;
window.SITE_CONSTANTS.WHATSAPP_GENERAL_MESSAGE = <?= json_encode($whatsappGeneralMessage, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP) ?>;
...
// ============================================================
// NOTIFICATION SYSTEM — Facebook-style sound + immediate alerts
// ============================================================
(function() {
    'use strict';
    ...
    var CONFIG = {
        refreshInterval: 5000,  // 5 seconds for near-instant detection
    ...
    // ============================================================
    // FACEBOOK-STYLE NOTIFICATION SOUND (louder, more prominent)
    // ============================================================
    function playFacebookSound() {
        try {
            ...
            // Facebook messenger uses a bright "pop/ding" — two quick tones
            ...
            // Second tone: higher ding at ~1245Hz (slightly delayed)
    ...
    if (toastEl) {
        toastEl.addEventListener('click', function(e) {
            ...
            ?>
            window.location.href = '<?php echo $msgLink; ?>';
        });
    }

    // ============================================================
    // UPDATE ALL BADGES (header, mobile drawer, hamburger button)
    // ============================================================
    function updateBadges() {
        ...
        .then(function(data) {
            if (data && data.success) {
                ...
                // NEW MESSAGE DETECTED — play sound + show toast
