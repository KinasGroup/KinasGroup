<!-- Contact Agent Modal -->
<?php
// GUARD DATA: know who is viewing so the modal can refuse
// self-contact and non-customer senders before any API call.
$kaModalUid  = (class_exists('SessionManager') && SessionManager::isLoggedIn()) ? (int)SessionManager::getUserId() : 0;
...
<script>
// GUARD DATA injected server-side (0 / '' when logged out).
...
// GUARD 1: the listing's own agent can never contact themselves.
if (KA_CURRENT_USER_ID && parseInt(agentId, 10) === KA_CURRENT_USER_ID) {
if (typeof kinasToast === 'function') kinasToast('This is your own listing — inquiries from yourself are not allowed.', 'warning');
return;
}
// GUARD 2: only customer accounts (or guests) may send inquiries.
if (KA_CURRENT_USER_ID && KA_CURRENT_USER_ROLE !== 'user') {
if (typeof kinasToast === 'function') kinasToast('Only customer accounts can send listing inquiries. Agents manage listings from their dashboard.', 'warning');
return;
}
...
e.preventDefault();
// Client-side mirror of the server guards (defense in depth).
...
if (KA_CURRENT_USER_ID && KA_CURRENT_USER_ROLE !== 'user') {
closeContactAgentModal();
if (typeof kinasToast === 'function') kinasToast('Only customer accounts can send listing inquiries.', 'warning');
return;
}
