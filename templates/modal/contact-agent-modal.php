...
// self-contact before any API call.
...
<script>
// GUARD DATA injected server-side.
var KA_CURRENT_USER_ID   = <?= (int)$kaModalUid ?>;
var KA_CURRENT_USER_ROLE = <?= json_encode($kaModalRole) ?>;

// Option A: customers, agents and admins may send listing inquiries.
...
function openContactAgentModal(listingId, listingType, agentId, agentName, agentVerified, agentDivision) {
    ...
    if (KA_CURRENT_USER_ID && KA_ALLOWED_INQUIRY_ROLES.indexOf(KA_CURRENT_USER_ROLE) === -1) {
        if (typeof kinasToast === 'function') kinasToast('Your account type cannot send listing inquiries.', 'warning');
        return;
    }
