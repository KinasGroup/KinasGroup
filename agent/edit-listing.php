...
$listingHardwareType = strtolower((string)($listing['hardware_type'] ?? 'solar_panel'));
$listingServiceType = strtolower((string)($listing['service_type'] ?? ''));
...
<div class="je-dash-shell" style="max-width:100%;overflow-x:hidden;">
    ...
    <main class="je-dash-main" style="overflow-x:hidden;width:100%;max-width:100%;padding:15px;">
        <div class="agent-container">
            ...
            <?php if ($flashSuccess): ?>
                <div style="background:#E8F5E9; border:1px solid #A5D6A7; color:#1B5E20; border-radius:8px; padding:14px 20px; margin-bottom:24px;">
                    <i class="fas fa-check-circle"></i> <?= htmlspecialchars($flashSuccess) ?>
                </div>
            <?php endif; ?>

            <?php if ($flashError): ?>
                <div style="background:#FFEBEE; border:1px solid #EF9A9A; color:#B71C1C; border-radius:8px; padding:14px 20px; margin-bottom:24px;">
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <div class="input-prefix">
                                    ...
                                    <input type="number" name="price" value="<?php echo htmlspecialchars($listing['price'] ?? 0); ?>" placeholder="0" step="0.01" required>
                                </div>
                            </div>
                        ...
                        <div id="automobileFields" style="display:<?php echo $divisionParam === 'car' ? 'block' : 'none'; ?>; margin-top:24px;">
                            ...
                            <div class="automobile-fields-grid">
                                <div class="form-group">
                                    <label><i class="fas fa-key"></i> Listing Purpose *</label>
                                    ...
                                    <?php if (!$canOfferRental): ?>
                                        <span style="display:block;font-size:12px;color:#888;margin-top:4px;">Rental listings are only available to verified registered businesses.</span>
                                    <?php endif; ?>
                                </div>

                                <div class="form-group">
                                    <label><i class="fas fa-clipboard-check"></i> Inspection Fee (₦) <span style="font-weight:400;color:#888;">(optional)</span></label>
                                ...
                                <div class="form-group">
                                    <label><i class="fas fa-tag"></i> Make *</label>
                                ...
                                <div class="form-group">
                                    <label><i class="fas fa-car-side"></i> Car Type</label>
                                ...
                                <div class="form-group">
                                    <label><i class="fas fa-steering-wheel"></i> Drive</label>
                                    <select name="drive">
                                ...
                                <div class="form-group">
                                    <label><i class="fas fa-road"></i> Drive Train</label>
                                ...
                                <div class="form-group">
                                    <label><i class="fas fa-clipboard-check"></i> Condition</label>
                                ...
                                <div class="form-group full-width">
                                    <label><i class="fas fa-check-circle"></i> Features (comma separated)</label>
                                    <?php
                                    $featuresDisplay = '';
                                    ...
                                    if (!empty($listing['features'])) {
                                        if (is_array($listing['features'])) {
                                            ...
                                            $featuresDisplay = is_array($decoded) ? implode(', ', $decoded) : (is_string($decoded)&&$decoded!==''?$decoded:$listing['features']);
                                        }
                                    }
                                    ?>
                            ...
                            <div style="margin-top:20px;">
                                ...
                                <div id="vtVideoField" class="form-group" style="display:<?php echo $vtType === 'video' ? '' : 'none'; ?>;">
                                    ...
                                    <span style="display:block;font-size:12px;color:#888;margin-top:4px;">
                                        MP4, MOV, or WebM — up to 150MB. <?php echo $vtType === 'video' && $vtUrl ? 'Leave blank to keep the current video.' : ''; ?>
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div id="solarFields" style="display:<?php echo $divisionParam === 'solar' ? 'block' : 'none'; ?>; margin-top:24px;">
                            ...
                            <div class="form-group">
                                <label><i class="fas fa-microchip"></i> Hardware Type *</label>
                                <select name="hardware_type" id="hardwareType">
                                    <option value="solar_panel" <?php echo $listingHardwareType === 'solar_panel' ? 'selected' : ''; ?>>Solar Panel</option>
                                    <option value="inverter" <?php echo $listingHardwareType === 'inverter' ? 'selected' : ''; ?>>Inverter</option>
                                    <option value="battery" <?php echo $listingHardwareType === 'battery' ? 'selected' : ''; ?>>Battery</option>
                                    <option value="power_station" <?php echo $listingHardwareType === 'power_station' ? 'selected' : ''; ?>>Power Station</option>
                                </select>
                            </div>

                            <div class="automobile-fields-grid">
                                <div class="form-group" id="panelWattsGroup">
                                    <label>Panel Capacity (W)</label>
                                    <input type="number" name="panel_watts" step="1" value="<?php echo htmlspecialchars($listing['panel_watts'] ?? ''); ?>" placeholder="e.g., 550">
                                </div>

                                <div class="form-group" id="inverterKvaGroup">
                                    <label>Inverter Capacity (kW/kVA)</label>
                                    <input type="number" name="inverter_kva" step="0.1" value="<?php echo htmlspecialchars($listing['inverter_kva'] ?? ''); ?>" placeholder="e.g., 5">
                                </div>

                                <div class="form-group" id="batteryKwhGroup">
                                    <label>Battery Capacity (kWh)</label>
                                    <input type="number" name="battery_kwh" step="0.1" value="<?php echo htmlspecialchars($listing['battery_kwh'] ?? ''); ?>" placeholder="e.g., 10">
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="solar_type">
                                        <option value="">Select Type</option>
                                        <option value="residential" <?php echo $listingServiceType === 'residential' ? 'selected' : ''; ?>>Residential</option>
                                        <option value="commercial" <?php echo $listingServiceType === 'commercial' ? 'selected' : ''; ?>>Commercial</option>
                                        <option value="industrial" <?php echo $listingServiceType === 'industrial' ? 'selected' : ''; ?>>Industrial</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-actions">
                            ...
                            <button type="submit" class="btn-submit" id="updateBtn">Update Listing</button>
                        </div>
                    </div>
        ...
        window.kinasConfirm = function(message, onConfirm, opts) {
            ...
            var variant=opts.variant||'danger',
                ...
                subtitle=opts.subtitle||'This action cannot be undone.',
                label=opts.confirm||'Delete',
                icon=opts.icon||'fa-trash-alt';

            var overlay=document.getElementById('kinasConfirmOverlay'),
                iconWrap=document.getElementById('kinasConfirmIconWrap'),
                badge=document.getElementById('kinasConfirmWarningBadge'),
                badgeTxt=document.getElementById('kinasConfirmWarningText'),
                proceedBtn=document.getElementById('kinasConfirmProceed'),
                proceedIcon=proceedBtn.querySelector('i'),
                proceedLbl=proceedBtn.querySelector('span');
            ...
            if(opts.warning){
                badgeTxt.textContent=opts.warning;
        ...
        function refreshCsrfToken(){
            const t=document.getElementById('csrfTokenInput');
            if(t) t.value=currentCsrfToken;

            document.querySelectorAll('input[name="csrf_token"]').forEach(i => i.value=currentCsrfToken);
        }
        ...
        function syncSolarHardware(){
            ...
            const show=(id,on)=>{
                ...
                if(elx) elx.style.display=on?'':'none';
            };
        ...
        function getPreviewUrl(f){
            let u=filePreviewUrls.get(f);

            if(!u){
                u=URL.createObjectURL(f);
                filePreviewUrls.set(f,u);
            }
        ...
        window.__kinasPreviewImgError=function(imgEl,index){
            ...
            const old=filePreviewUrls.get(f);

            if(old){
                URL.revokeObjectURL(old);
                filePreviewUrls.delete(f);
            }

            const fresh=getPreviewUrl(f);
            ...
            imgEl.onerror=function(){
                window.__kinasPreviewImgError(imgEl,index);
            };
        ...
        function renderImages(){
            ...
            existingImages.forEach((img,index)=>{
                ...
                div.innerHTML=`<img src="${img.url}" alt="Existing image ${index+1}"><span class="preview-badge">Saved</span><button type="button" class="preview-remove" onclick="removeExistingImage(${img.id}, this)" title="Remove this image">&times;</button>`;
                imagePreviewGrid.appendChild(div);
            });

            const cur=new Set(selectedFiles);

            for(const [f,u] of filePreviewUrls){
                if(!cur.has(f)){
            ...
            selectedFiles.forEach((f,index)=>{
                ...
                div.innerHTML=`<img src="${getPreviewUrl(f)}" alt="New image ${index+1}" onerror="window.__kinasPreviewImgError && window.__kinasPreviewImgError(this, ${index})"><button type="button" class="preview-remove" onclick="removeNewImage(${index})" title="Remove this image">&times;</button>`;
                imagePreviewGrid.appendChild(div);
            });
        }
        ...
        function removeExistingImage(imageId,button){
            kinasConfirm('Remove this image from the listing?',function(){
                ...
                const csrf=document.getElementById('csrfTokenInput')?.value||currentCsrfToken;

                fetch('/api/listings/delete-image.php',{
                    method:'POST',
                    headers:{'Content-Type':'application/json'},
                    body:JSON.stringify({image_id:imageId,csrf_token:csrf})
                })
                .then(r=>{
                ...
                .then(d=>{
                    if(d.success){
                        const i=existingImages.findIndex(x=>x.id===imageId);
                        ...
                        kinasToast(d.error||'Failed to remove image','error');
                        button.classList.remove('loading');
                        button.innerHTML='&times;';

                        if(d.error && d.error.toLowerCase().includes('security token')){
                            setTimeout(()=>window.location.reload(),1000);
                        }
                    }
                })
                .catch(e=>{
                    kinasToast('Error: '+e.message,'error');
                    button.classList.remove('loading');
                    button.innerHTML='&times;';
                });
        ...
        if(imageUpload){
            imageUpload.addEventListener('kinas:images-ready',function(e){
                const nf=(e.detail && Array.isArray(e.detail.files)) ? e.detail.files : Array.from(imageUpload.files||[]);
        ...
        document.addEventListener('DOMContentLoaded',function(){
            setTimeout(function(){
                if(imagePreviewGrid && imagePreviewGrid.children.length===0) renderImages();
            },100);
        });

        function showToast(m,t){
            var map={success:'success',error:'error',info:'info'};
            kinasToast(m,map[t]||'info');
        }
        </script>
    </main>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    vtTypeLink.addEventListener('change',sync);
    vtTypeVideo.addEventListener('change',sync);
});
</script>
