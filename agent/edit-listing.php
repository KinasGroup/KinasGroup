...
<?php
// REVAMP (features JSON fix): the DB column is JSON. Old rows may hold
// either a JSON array ("[\"A\",\"B\"]"), a JSON string scalar
// ("\"A, B\"") or — for very old car rows — a raw comma string.
// Decode whatever is there and always present a clean comma-separated
// list to the text input (no literal quotes / brackets).
$featuresDisplay = '';
if (!empty($listing['features'])) {
if (is_array($listing['features'])) {
$featuresDisplay = implode(', ', $listing['features']);
} else {
$decoded = json_decode((string)$listing['features'], true);
if (is_array($decoded)) {
$featuresDisplay = implode(', ', array_map('strval', $decoded));
} elseif (is_string($decoded)) {
// JSON string scalar (old property/solar path) — use the
// decoded text, NOT the raw quoted DB value.
$featuresDisplay = $decoded;
} else {
// Not JSON at all — show as-is.
$featuresDisplay = (string)$listing['features'];
}
}
}
?>
