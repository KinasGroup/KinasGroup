...
.agent-header h1{font-family:'Prata',serif;font-size:28px;color:#0A0A0A;} .agent-header h1 i{color:#C6A43F;margin-right:12px;}
.btn-secondary{background:#F5F5F5;color:#666;padding:10px 20px;border-radius:40px;text-decoration:none;display:inline-flex;align-items:center;gap:8px;transition:all .3s;border:1px solid #E0E0E0;}
...
.form-section{padding:32px;} .form-section:first-child{border-right:1px solid #E0E0E0;}
...
.form-group{margin-bottom:24px;} .form-group label{display:block;margin-bottom:8px;font-size:13px;font-weight:600;color:#333;} .form-group label i{color:#C6A43F;margin-right:6px;}
.form-group input,.form-group select,.form-group textarea{width:100%;padding:12px 16px;border:1px solid #E0E0E0;border-radius:12px;font-family:'Inter',sans-serif;font-size:14px;transition:all .3s;background:#fff;}
...
.input-prefix{position:relative;display:flex;align-items:center;} .prefix{position:absolute;left:16px;color:#C6A43F;font-weight:600;} .input-prefix input{padding-left:32px;}
.image-upload-area{border:2px dashed #E0E0E0;border-radius:16px;padding:20px;text-align:center;transition:all .3s;position:relative;}
.image-upload-area:hover{border-color:#C6A43F;background:rgba(198,164,63,0.02);}
.upload-placeholder{cursor:pointer;} .upload-placeholder i{font-size:48px;color:#C6A43F;margin-bottom:12px;} .upload-placeholder p{margin-bottom:8px;color:#666;} .upload-placeholder span{font-size:12px;color:#999;}
.image-preview-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(100px,1fr));gap:12px;margin-top:20px;}
.preview-item{position:relative;border-radius:12px;overflow:hidden;aspect-ratio:1;background:#F5F5F5;} .preview-item img{width:100%;height:100%;object-fit:cover;}
...
.checkbox-group{display:flex;align-items:center;gap:12px;margin-top:8px;} .checkbox-label{display:flex;align-items:center;gap:10px;cursor:pointer;} .checkbox-label input{width:auto;accent-color:#C6A43F;}
...
.btn-submit{padding:12px 32px;background:#C6A43F;border:none;border-radius:40px;font-weight:600;color:#0A0A0A;cursor:pointer;transition:all .3s;}
.btn-submit:hover{background:#A8882E;transform:translateY(-2px);}
.automobile-fields-grid{display:grid;grid-template-columns:1fr 1fr;gap:16px;} .automobile-fields-grid .form-group{margin-bottom:0;}
...
@media (prefers-color-scheme: dark){body{background:#F5F7FA !important;}.agent-header h1{color:#0A0A0A !important;}.listing-form{background:white !important;}.form-section h3{color:#C6A43F !important;}.form-group label{color:#333 !important;}.form-group input,.form-group select,.form-group textarea{background:#fff !important;}.btn-cancel{background:#F5F5F5 !important;color:#666 !important;}.btn-submit{background:#C6A43F !important;color:#0A0A0A !important;}}
</style>
...
<?php foreach ($divisionMap as $dbDiv => $info): $sel = (!$isSuperAgent && $dbDiv === $agentDivision) ? 'selected' : ''; ?>
<option value="<?= htmlspecialchars($info['opt']) ?>" <?= $sel ?>><?= htmlspecialchars($info['label']) ?></option>
<?php endforeach; ?>
</select>
<?php if (!$isSuperAgent && $agentDivision): ?><small style="color:#666;font-size:12px;">Your account is registered for <strong><?= htmlspecialchars($divisionMap[$agentDivision]['label'] ?? $agentDivision) ?></strong> only.</small><?php endif; ?>
...
<select name="car_listing_type" id="carListingType" required><option value="sale">For Sale</option><?php if ($canOfferRental): ?><option value="rental">For Rental</option><?php endif; ?></select>
...
<div class="form-group"><label>Make *</label><select name="brand" required><option value="">Select Make</option><option>Toyota</option><option>Mercedes-Benz</option><option>BMW</option><option>Audi</option><option>Lexus</option><option>Honda</option><option>Ford</option><option>Hyundai</option><option>Kia</option><option>Nissan</option><option>Porsche</option><option>Land Rover</option><option>Tesla</option><option>Other</option></select></div>
<div class="form-group"><label>Model *</label><input type="text" name="model" placeholder="e.g., S-Class" required></div>
<div class="form-group"><label>Year *</label><input type="number" name="year" min="1900" max="2099" required></div>
...
<div class="form-group"><label>VIN</label><input type="text" name="vin"></div>
<div class="form-group"><label>Color</label><input type="text" name="color"></div>
<div class="form-group"><label>Interior Color</label><input type="text" name="interior_color"></div>
...
<div class="form-group"><label>Bedrooms</label><input type="number" name="bedrooms"></div>
<div class="form-group"><label>Bathrooms</label><input type="number" name="bathrooms"></div>
<div class="form-group"><label>Area (sq ft)</label><input type="text" name="area"></div>
...
<div class="form-group"><label>Inspection Fee (₦)</label><input type="number" name="inspection_fee" min="0" step="0.01"></div>
</div>
...
<div class="form-group"><label>System Type</label><select name="solar_type"><option value="">Select Type</option><option value="Residential">Residential</option><option value="Commercial">Commercial</option><option value="Industrial">Industrial</option></select></div>
</div>
</div>
...
if (d==='solar') syncSolarHardware();
}
document.addEventListener('DOMContentLoaded', function(){
syncListingType();
const divisionSelect=document.getElementById('division');
if(divisionSelect) divisionSelect.addEventListener('change', syncListingType);
const hardwareSelect=document.getElementById('hardwareType');
if(hardwareSelect){ hardwareSelect.addEventListener('change', syncSolarHardware); syncSolarHardware(); }
const vtTypeLink=document.getElementById('vtTypeLink'), vtTypeVideo=document.getElementById('vtTypeVideo'), vtLinkField=document.getElementById('vtLinkField'), vtVideoField=document.getElementById('vtVideoField'), vtUrlInput=document.getElementById('vtUrlInput'), vtVideoInput=document.getElementById('vtVideoInput');
function syncVirtualTourMode(){ if(!vtTypeLink||!vtTypeVideo)return; const isLink=vtTypeLink.checked; vtLinkField.style.display=isLink?'':'none'; vtVideoField.style.display=isLink?'none':''; if(isLink&&vtVideoInput)vtVideoInput.value=''; if(!isLink&&vtUrlInput)vtUrlInput.value=''; }
...
document.addEventListener('DOMContentLoaded', function(){
const form=document.getElementById('listingForm'); const submitBtn=document.getElementById('submitBtn'); let isSubmitting=false;
if(form&&submitBtn){ form.addEventListener('submit', function(e){ e.preventDefault(); if(isSubmitting)return;
...
const imageUpload=document.getElementById('imageUpload'); const previewGrid=document.getElementById('imagePreviewGrid'); let selectedFiles=[]; const filePreviewUrls=new Map();
...
function updatePreview(){ if(!previewGrid)return; previewGrid.innerHTML=''; const cur=new Set(selectedFiles); for(const [f,u] of filePreviewUrls){ if(!cur.has(f)){URL.revokeObjectURL(u);filePreviewUrls.delete(f);} } selectedFiles.forEach((f,i)=>{ const d=document.createElement('div'); d.className='preview-item'; d.innerHTML=`<img src="${getPreviewUrl(f)}" onerror="window.__kinasPreviewImgError && window.__kinasPreviewImgError(this, ${i})"><button type="button" class="preview-remove" onclick="removeImage(${i})">&times;</button>`; previewGrid.appendChild(d); }); }
