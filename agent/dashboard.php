...
*
* AMENDED: welcome banner now shows the agent's public @username.
*/
...
// AMENDED: resolve the agent's public username for the welcome banner.
try {
$stmt = $db->prepare("SELECT username, name FROM users WHERE id = ? LIMIT 1");
$stmt->execute([$agentId]);
$agentUser = $stmt->fetch(PDO::FETCH_ASSOC) ?: [];
} catch (Exception $e) {
$agentUser = [];
}
$agentDisplayName = kinas_public_display_name(
$agentUser['username'] ?? null,
...
<p>Welcome back, <?php echo htmlspecialchars($agentDisplayName); ?>! (Agent ID: <?php echo $agentId; ?>)</p>
</div>
</div>
<!-- Flash Messages -->
